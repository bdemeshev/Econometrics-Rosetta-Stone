...
\[
\hat{lnpricemetr_i} = \hat{\beta_0} + \hat{\beta}_{lnkitsp} \cdot lnkitsp_i + \hat{\beta}_{livesp}\cdot livesp_i + \hat{\beta}_{lndist}\cdot lndist_i + \hat{\beta}_{lnmetrdist}\cdot lnmetrdist_i
e_1, …, e_n.
\]
На втором - строится вспомогательная регрессия (X_j-вектор j-го фактора).
\[
\hat{e^2_i} = \hat{\alpha_0} + \sum_{j=1}^{k} \hat{\alpha_j} \cdot X_j + \sum_{j=1}^{k} \hat{\gamma_j} \cdot X^2_j + \sum_{j < m}^{k} \hat{\delta_j} X_j \cdot X_m
\]
...
\[
H_0: \sigma^2_i = \sigma^2\\
H_1: \sigma^2_i \propto f(\alpha_0 + \alpha_1 \cdot Z_1 +  ... + \alpha_p \cdot Z_p )
\]
...
\[
\frac{e^2}{\hat{\sigma}^2} = \alpha_0 + \alpha_1 \cdot Z_1 + ... + \alpha_p \cdot Z_p + u
\]
...
```
Тест Голдфелда-Квандта результаты??????

Будет также полезным познакомиться с методами борьбы с гетероскедастичностью.

Способ 1. Взвешенный МНК. Веса – обратная дисперсия переменной, вызывающей гетероскедачность.
То есть оценим регрессию:
\[
\frac{\hat{lnpricemetr_i}}{\hat{\sigma_i}} = \frac{\hat{\beta_0}{{\hat{\sigma_i}} + \frac{\hat{\beta}_{lnkitsp} \cdot lnkitsp_i}{\sigma_i} +\frac{ \hat{\beta}_{livesp}\cdot livesp_i}{\sigma_i} + \frac{\hat{\beta}_{lndist}\cdot lndist_i}{\sigma_i} + \frac{\hat{\beta}_{lnmetrdist}\cdot lnmetrdist_i}{\sigma_i}
\]
