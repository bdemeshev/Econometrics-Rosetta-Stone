...
ggplot(data, aes(math, apt, col = prog)) + geom_point()
```
...
```{r}
data = import('data_alcohol_tobacco.dta')
summary(data)
...
```{r}
qplot(data, x = shalc) + ylab("Frequency") + xlab("Share of alcohol")
# ggplot(data, aes(x = shalc)) + geom_histogram() + ylab("Frequency") + xlab("Share of alcohol")

```


```{r}
heck1 = heckit(alc ~ age + nadults + nkids + lnx + walloon, 
               shalc ~  age + nadults + nkids + lnx + walloon,
               data = data,
               method = 'ml')
summary(heck1)
```
...
```{r}
heck2 = heckit2fit(alc ~ age + nadults + nkids + lnx + walloon,
                   shalc ~  age + nadults + nkids + lnx + walloon,
                   data = data)
summary(heck2)
```
